(() => {
  ...
  function addBundleCard(grid,withLink){
    ...
    article.innerHTML=`${icon}<h3>Bundle</h3><p>${es?'Bootstrapper WiX Burn que encadena prerequisitos EXE/MSI y el MSI principal en un único ejecutable.':'WiX Burn bootstrapper that chains EXE/MSI prerequisites and the main MSI into one executable.'}</p>${withLink?`<a href="docs/#bundle">${es?'Más información →':'Learn more →'}</a>`:''}`;
