(() => {
  ...
  const topicUrl=number=>`./topic/?id=${encodeURIComponent(number)}`;
  ...
  fetch(`${FEED}?v=${Date.now()}`,{cache:'no-store'})
    ...
    .then(payload=>{
      posts=(payload.discussions||[]).map(item=>({number:item.number,title:cleanTitle(item.title),url:item.url,user:item.user||'developer',avatar:item.avatar||'',comments:item.comments||0,updated:item.updatedAt||item.createdAt,category:categoryFrom(item),summary:item.bodyText||'',answered:Boolean(item.answered)}));
