(() => {
  ...
  const copy = {
    en:{
      kicker:'InstallerLab interface',
      title:'20 interface languages. Up to 36 languages in the installers you build.',
      lede:'The language used by InstallerLab itself is independent from the languages included in your generated installers. Use InstallerLab in any of its 20 supported interface languages, then choose from up to 36 installer languages for the setup you create.',
      fallback:'If the operating system uses a language that InstallerLab does not support, the application safely falls back to English. This does not limit the languages you can include in the installer you generate.',
      ui:'interface languages',setup:'installer languages',community:'Developer experiences',communityTitle:'Used InstallerLab? Rate it and tell other developers what you built.',communityBody:'Public reviews are submitted through GitHub Issues. Ratings and experiences can be read by anyone, so feedback stays transparent and easy to verify.',rate:'Rate InstallerLab',all:'See all experiences',ratings:'ratings',none:'No public ratings yet',first:'Be the first to share your experience with InstallerLab.',loading:'Loading public reviews…',verified:'Public GitHub review',source:'Reviews shown here come from public GitHub Issues.',fallbackLabel:'English fallback:'
    },
    es:{
      kicker:'Interfaz de InstallerLab',
      title:'20 idiomas de interfaz. Hasta 36 idiomas en los instaladores que creas.',
      lede:'El idioma utilizado por InstallerLab es independiente de los idiomas incluidos en los instaladores generados. Puedes usar InstallerLab en cualquiera de sus 20 idiomas de interfaz y después elegir entre hasta 36 idiomas para el instalador que estás creando.',
      fallback:'Si Windows utiliza un idioma que InstallerLab todavía no soporta, la aplicación usa inglés como idioma de respaldo. Esto no limita los idiomas que puedes incluir en el instalador generado.',
      ui:'idiomas de interfaz',setup:'idiomas para instaladores',community:'Experiencias de desarrolladores',communityTitle:'¿Usaste InstallerLab? Califícalo y cuéntale a otros desarrolladores qué creaste.',communityBody:'Las opiniones públicas se envían mediante GitHub Issues. Cualquiera puede leer la calificación y la experiencia, por lo que el feedback permanece transparente y verificable.',rate:'Calificar InstallerLab',all:'Ver todas las experiencias',ratings:'calificaciones',none:'Aún no hay calificaciones públicas',first:'Sé la primera persona en compartir tu experiencia con InstallerLab.',loading:'Cargando opiniones públicas…',verified:'Opinión pública en GitHub',source:'Las opiniones mostradas aquí provienen de GitHub Issues públicos.',fallbackLabel:'Respaldo en inglés:'
    }
  };

  function currentLang(){
    return (localStorage.getItem('il-lang') || 'en').toLowerCase() === 'es' ? 'es' : 'en';
  }
  ...
  function stars(n){
    return `<span class="il-stars" aria-label="${n} out of 5">${'★'.repeat(n)}${'☆'.repeat(5-n)}</span>`;
  }
  ...
  function stateKey(){
    return `${currentLang()}|${loaded ? 1 : 0}|${reviews.length}`;
  }
  ...
  function markup(){
    ...
    return `<section id="il-language-community" class="il-language-community" data-render-state="${esc(stateKey())}">
      <article class="il-language-card">
        ...
        <p class="il-language-lede"><strong>${l.fallbackLabel}</strong> ${l.fallback}</p>
      </article>
  ...
  function mount(force = false){
    ...
    // Critical performance guard: do not rewrite our own section when nothing changed.
    // The previous implementation replaced outerHTML on every DOM mutation, which
    // caused its MutationObserver to trigger itself forever.
    if (existing && !force && existing.dataset.renderState === nextState) return;

    const html = markup();
    if (existing) existing.outerHTML = html;
    else footer.insertAdjacentHTML('beforebegin', html);
  }
  ...
  function start(){
    ...
    // site.js builds/rebuilds the page dynamically. Observe it only to remount if
    // that renderer removes our section or if the selected site language changes.
    // queueMount + data-render-state prevents a self-triggered render loop.
    const observer = new MutationObserver(queueMount);
    observer.observe(document.body,{subtree:true,childList:true});

    // storage fires when another tab changes the setting; same-tab language changes
    // also produce DOM mutations through the main site renderer and are caught above.
